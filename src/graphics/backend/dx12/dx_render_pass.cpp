...
namespace {
constexpr size_t k_inline_barrier_capacity = 8u;
}
...
void dx_render_pass_impl::dx_render_pass_bind(const render_pass& _render_pass, const command_buffer& _command_buffer, const framebuffer& _framebuffer, const depth_buffer* _depth_buffer, const render_pass_bind_param& _params) {
	...
	std::array<D3D12_RESOURCE_BARRIER, k_inline_barrier_capacity> inline_barriers = {};
	std::vector<D3D12_RESOURCE_BARRIER> heap_barriers;
	D3D12_RESOURCE_BARRIER* barriers = nullptr;
	if (fb_data->render_targets.size() <= inline_barriers.size()) {
		barriers = inline_barriers.data();
	}
	else {
		heap_barriers.resize(fb_data->render_targets.size());
		barriers = heap_barriers.data();
	}
	size_t barrier_count = 0;
	for (size_t i = 0; i < fb_data->render_targets.size(); ++i) {
		D3D12_RESOURCE_BARRIER barrier = {};
		barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		barrier.Transition.pResource = fb_data->render_targets[i].Get();
		barrier.Transition.StateBefore = fb_data->is_swapchain
			? D3D12_RESOURCE_STATE_PRESENT
			: (fb_data->render_target_textures[i] ? fb_data->render_target_textures[i]->dx12_state : fb_data->before_render_states[i]);
		barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
		barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		barriers[barrier_count++] = barrier;
		if (!fb_data->is_swapchain && fb_data->render_target_textures[i])
			fb_data->render_target_textures[i]->dx12_state = D3D12_RESOURCE_STATE_RENDER_TARGET;
	}
	if (barrier_count != 0)
		cb_data->cmd_list->ResourceBarrier(static_cast<UINT>(barrier_count), barriers);

	float clear_color[] = {_params.clear_color.x, _params.clear_color.y, _params.clear_color.z, _params.clear_color.w};
	for (const auto& rtv_handle : fb_data->rtv_handles)
		cb_data->cmd_list->ClearRenderTargetView(rtv_handle, clear_color, 0, nullptr);
	...
	if (depth_data != nullptr) {
		...
		cb_data->cmd_list->OMSetRenderTargets(static_cast<UINT>(fb_data->rtv_handles.size()), fb_data->rtv_handles.data(), TRUE, &depth_data->dsv_handle);
	} else {
		cb_data->cmd_list->OMSetRenderTargets(static_cast<UINT>(fb_data->rtv_handles.size()), fb_data->rtv_handles.data(), TRUE, nullptr);
	}
}
...
void dx_render_pass_impl::dx_render_pass_unbind(const render_pass& _render_pass, const command_buffer& _command_buffer) {
	...
	for (size_t i = 0; i < fb_data->render_targets.size(); ++i) {
		...
		if (!fb_data->is_swapchain) {
			fb_data->before_render_states[i] = after_state;
			if (fb_data->render_target_textures[i])
				fb_data->render_target_textures[i]->dx12_state = after_state;
		}
	}
	if (barrier_count != 0)
		cb_data->cmd_list->ResourceBarrier(static_cast<UINT>(barrier_count), barriers);
