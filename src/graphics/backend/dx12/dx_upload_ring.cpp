...
mars::upload_ring dx_upload_ring_impl::create(const mars::device& _device, size_t capacity) {
	...
	auto* copy_qdata = device_data->copy_queue_data.expect<dx_command_queue_data>();
	...
	hr = dx_expect<&ID3D12Device::CreateFence>(device_data->device.Get(), 0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&d->fence));
	...
	hr = dx_expect<&ID3D12Device::CreateCommandAllocator>(device_data->device.Get(), D3D12_COMMAND_LIST_TYPE_COPY, IID_PPV_ARGS(&d->allocator));

	hr = dx_expect<&ID3D12Device::CreateCommandList>(
		device_data->device.Get(), 0, D3D12_COMMAND_LIST_TYPE_COPY, d->allocator.Get(), nullptr, IID_PPV_ARGS(&d->cmd_list)
	);
...
void dx_upload_ring_impl::schedule(mars::upload_ring& _ring, const mars::buffer& dst, size_t dst_offset, const void* src, size_t size) {
	auto* d = _ring.data.expect<dx_upload_ring_data>();
	auto* buf_data = dst.data.expect<dx_buffer_data>();
	...
	logger::error_if(aligned_size > d->capacity, ring_channel, "upload ring schedule exceeds ring capacity (size={}, aligned={}, capacity={})", size, aligned_size, d->capacity);
	assert(aligned_size <= d->capacity && "upload ring request exceeds capacity");

	logger::error_if(dst_offset + size > buf_data->size, ring_channel, "upload ring schedule exceeds destination buffer (dst_offset={}, size={}, buffer_size={})", dst_offset, size, buf_data->size);
	assert(dst_offset + size <= buf_data->size && "upload ring destination range out of bounds");

	logger::error_if(d->batch_start > d->write_head, ring_channel, "upload ring batch state corrupted (batch_start={}, write_head={})", d->batch_start, d->write_head);
	...
	logger::error_if(upload_region_overlaps(d, offset, offset + aligned_size), ring_channel, "upload ring scheduled overlapping in-flight region (offset={}, aligned={})", offset, aligned_size);
...
uint64_t dx_upload_ring_impl::flush(mars::upload_ring& _ring) {
	...
	logger::error_if(d->batch_start > d->write_head, ring_channel, "upload ring flush observed invalid batch range (batch_start={}, write_head={})", d->batch_start, d->write_head);
