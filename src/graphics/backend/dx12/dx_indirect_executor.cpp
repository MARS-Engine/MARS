...
indirect_executor create_with_constant_impl(
	...
	logger::assert_(pipe_data != nullptr, ie_channel, "create_with_constant: pipeline.data is null");
	logger::error_if(!pipe_data->has_push_constants, ie_channel, "create_with_constant: pipeline has no push_constants root parameter");
	...
	dx_expect<&ID3D12Device::CreateCommandSignature>(
		dev_data->device.Get(), &sig_desc, pipe_data->root_signature.Get(), IID_PPV_ARGS(&data->command_signature)
	);
	ex.data.store(data);
	return ex;
}
...
indirect_executor dx_indirect_executor_impl::create_with_constant(const device& dev, const pipeline& pipe) {
	auto pipe_data = pipe.data.expect<dx_pipeline_data>();
...
indirect_executor dx_indirect_executor_impl::create_with_constant_compute(const device& dev, const compute_pipeline& pipe) {
	auto pipe_data = pipe.data.expect<dx_compute_pipeline_data>();
	return create_with_constant_impl(
		dev,
		pipe_data,
...
indirect_executor dx_indirect_executor_impl::create(const device& dev, mars_command_signature_type type) {
	...
	dx_expect<&ID3D12Device::CreateCommandSignature>(
		dev_data->device.Get(), &sig_desc, nullptr, IID_PPV_ARGS(&data->command_signature)
	);
...
static void do_record(
	...
	auto data = ex.data.expect<dx_indirect_executor_data>();
	auto cb_data = cmd.data.expect<dx_command_buffer_data>();
	auto arg_data = arg_buf.data.expect<dx_buffer_data>();
	...
	if (count_buf) {
		auto count_data = count_buf->data.expect<dx_buffer_data>();
