...
class bound_raster_set {
	...
	void draw_indexed(mars::mesh_data<MeshLayoutT>& mesh) {
		static_assert(std::is_same_v<LayoutT, MeshLayoutT>,
			      "draw_indexed(mesh): mesh layout must match the currently bound descriptor set layout");
...
class compute_scope {
	...
	void bind_set(const mars::descriptor_set& set,
		      const mars::compute_pipeline& pl, size_t frame = 0) {
	...
	void set_push_constants(const mars::compute_pipeline& pl, const T& pc) {
		mars::graphics::object::command_buffer_set_compute_push_constants(*m_cmd, pl, pc);
	}

	void transition(mars::texture& tex,
...
class raster_scope {
	...
	void assert_expected_extent(const mars::vector2<size_t>& extent) {
		if constexpr (mars::graphics::object::pass_desc_traits<PassTag>::uses_swapchain)
			mars::logger::assert_(extent.x == m_window_size.x && extent.y == m_window_size.y,
					      mars::log_channel("framebuffer"),
					      "swapchain framebuffer extent mismatch: got {}x{}, expected {}x{}",
					      extent.x, extent.y, m_window_size.x, m_window_size.y);
		...
		if constexpr (mars::graphics::object::pass_desc_traits<PassTag>::has_fixed_size) {
			...
			mars::logger::assert_(extent.x == expected.x && extent.y == expected.y,
					      mars::log_channel("framebuffer"),
					      "fixed-size framebuffer extent mismatch: got {}x{}, expected {}x{}",
					      extent.x, extent.y, expected.x, expected.y);
		}
	}
	...
	~raster_scope() {
		if (m_began) mars::graphics::render_pass_unbind(*m_rp, *m_cmd);
	}

	raster_scope(const raster_scope&) = delete;
	raster_scope& operator=(const raster_scope&) = delete;

	void begin(const mars::graphics::object::framebuffer_handle<PassTag>& fb,
		   const mars::render_pass_bind_param& params) {
		static_assert(!mars::graphics::object::pass_desc_traits<PassTag>::uses_swapchain,
			      "raster_scope::begin(framebuffer_handle): PassTag uses swapchain; use swapchain_framebuffer_handle instead");
	...
	void begin(const mars::graphics::object::swapchain_framebuffer_handle<PassTag>& fb,
		   const mars::render_pass_bind_param& params) {
		static_assert(mars::graphics::object::pass_desc_traits<PassTag>::uses_swapchain,
			      "raster_scope::begin(swapchain_framebuffer_handle): PassTag is not swapchain-backed");
	...
	template <typename LayoutT>
	bound_raster_set<LayoutT> bind_set(const mars::owned_descriptor_set<LayoutT>& set,
					   const mars::pipeline& pl, size_t frame = 0) {
	...
	void set_push_constants(const mars::pipeline& pl, const T& pc) {
		mars::graphics::object::command_buffer_set_push_constants(*m_cmd, pl, pc);
	}
