...
class compute_builder {
  ...
  public:
		...
		template for (constexpr auto mem :
			...
			if constexpr (meta::has_annotation<graphics::compute_input>(mem)) {
				...
				if constexpr (std::is_same_v<C, texture>) {
					...
					++m_pool_params.descriptors_size[MARS_DESCRIPTOR_TYPE_SAMPLED_IMAGE];
				} else {
	...
	void dispatch(const command_buffer& cmd, const data_type& data, const command_buffer_dispatch_params& group_count) {
	...
	void dispatch_pc(const command_buffer& cmd, const data_type& data, const PC& push_consts, const command_buffer_dispatch_params& group_count) {
		const descriptor_set set = build_set(data);
		graphics::compute_pipeline_bind(m_pipeline, cmd);
		mars::graphics::descriptor_set_bind_compute(set, cmd, m_pipeline, 0);
		mars::graphics::command_buffer_set_compute_push_constants(cmd, m_pipeline, push_consts);
		graphics::command_buffer_dispatch(cmd, group_count);
	}
