...
namespace mvre_debug {
    ...
    public:
        ...
        static inline void error(const char* message) { std::cout << "\033[31m[error] \033[0m" << message << std::endl; throw std::runtime_error("\033[31m[error] \033[0m" + (std::string)message); }
        static inline void error(const std::string& message) { error(message.c_str()); }
    };
}
